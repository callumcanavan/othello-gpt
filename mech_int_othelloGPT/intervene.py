...
def intervene(probe_dir, layers, hook, device="mps"):
    ...
    for _sample in intv_data:
        ...
        padding = torch.zeros(2).to(device)
    ...
    return np.mean(errs), np.mean(null_intv_errs)


def intervene_layer_hooks(pre_probe_dir, post_probe_dir, layer_hooks, device="mps"):
    # print(f"Pre probe dir: {pre_probe_dir}")
    # print(f"Post probe dir: {post_probe_dir}.")
    # print(f"Layer hooks: {layer_hooks}")

    pre_probes = {}
    for layer in range(8):
        pre_probes[layer] = torch.load(
            os.path.join(pre_probe_dir, f"resid_{layer}_linear.pth"), map_location=torch.device(device)
        )
        pre_probes[layer].requires_grad = False
    
    post_probes = {}
    for layer in range(8):
        post_probes[layer] = torch.load(
            os.path.join(post_probe_dir, f"resid_{layer}_linear.pth"), map_location=torch.device(device)
        )
        post_probes[layer].requires_grad = False
    ...
    def patch_heads(
        ...
        add_vector = None
        if "resid_mid" in hook.name:
            probe = pre_probes[probe_layer]
        elif "resid_post" in hook.name:
            probe = post_probes[probe_layer]
        else:
            raise RuntimeError("Unexpected hook name.")
        
        if move_idx % 2 == 0:
            # White's turn to play, flipping white's cell.
            if from_player == 0:
                add_vector = probe[0, ..., row, col, 2]
            elif from_player == 2:
                add_vector = probe[0, ..., row, col, 1]
            else:
                raise RuntimeError("Unexpected from_player value.")
    ...
    for _sample in intv_data:
        ...
        (patched_logits, modified_cache) = run_with_cache_and_hooks(
            othello_gpt,
            list((f"blocks.{layer}.hook_{hook}", hook_fns[layer]) for layer, hook in layer_hooks),
            partial_game,
        )
