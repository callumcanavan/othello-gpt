    ...
    for layer in range(8):
        ...
        for epoch in range(num_epochs):
            ...
            for idx in pbar:
                ...
                # Sanity check
                # attn_out = cache["attn_out", layer][:, pos_start:pos_end]
                # resid_pre = cache["resid_pre", layer][:, pos_start:pos_end]
                # ic(resid_mid.shape)
                # ic(attn_out.shape)
                # ic(resid_pre.shape)
                # ic(torch.mean(resid_mid - resid_pre - attn_out))
                # assert torch.allclose(resid_mid, resid_pre + attn_out)
                ...
                if idx % valid_every == 0:
                    ...
                    for val_batch_idx in range(0, valid_size, batch_size):
                        ...
                        val_resid_mid = _val_cache["resid_mid", layer][
                            :, pos_start:pos_end
                        ]
    ...
    for layer in range(8):
        ...
        for test_layer in range(layer + 1):
            ...
            for idx in range(0, test_size, batch_size):
                ...
                resid_mid = cache["resid_mid", test_layer][
                    :, pos_start:pos_end
                ]
                probe_out = einsum(
                    "batch pos d_model, modes d_model rows cols options -> modes batch pos rows cols options",
